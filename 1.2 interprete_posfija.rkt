    ...
    (cases primitive prim
      (add-prim ()
                (if(equal? (length args) 2)
                   (+ (car args) (cadr args))
                         (eopl:error 'add-prim
              "Error con el numero de parametros se esperaban 2 y se estan recibiendo ~s" (length args))))
      (substract-prim ()
                      (if(equal? (length args) 2)
                         (- (car args) (cadr args))
                               (eopl:error 'substract-prim
              "Error con el numero de parametros se esperaban 2 y se estan recibiendo ~s" (length args))))
      (mult-prim ()
                 (if(equal? (length args) 2)
                 (* (car args) (cadr args))
                 (eopl:error 'multi-prim
              "Error con el numero de parametros")))
      (div-prim ()
                (if(equal? (length args) 2)
                   (/ (car args) (cadr args))
                   (eopl:error 'div-prim
      ...
      (incr-prim ()
                 (if(equal? (length args) 1)
                    (+ (car args) 1)
                    (eopl:error 'incr-prim
              "Error con el numero de parametros se esperaba 1 y se estan recibiendo ~s" (length args))))
      (decr-prim ()
                 (if(equal? (length args) 1)
                    (- (car args) 1)
                    (eopl:error 'decr-prim
              "Error con el numero de parametros se esperaba 1 y se estan recibiendo ~s" (length args)))))))
